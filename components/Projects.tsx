      ...
      <AnimatePresence>
        {selectedProject && (
          ...
          >
            ...
            <motion.div
              className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50"
            ...
            >
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-3 right-3 z-10 p-2 rounded-full bg-gray-100/80 dark:bg-gray-700/80 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all backdrop-blur-sm"
              >
              ...
              <div className="p-4 sm:p-6">
                <h2 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 text-gray-900 dark:text-white line-clamp-2">
                  {selectedProject?.title ?? "Untitled Project"}
                </h2>

                <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base leading-relaxed mb-4 sm:mb-5 line-clamp-3">
                  {selectedProject?.description}
                </p>

                {Array.isArray(selectedProject?.technologies) && selectedProject.technologies.length > 0 && (
                  <div className="mb-5 sm:mb-6">
                    <h3 className="text-sm sm:text-base font-bold mb-2.5 text-gray-900 dark:text-white">Tech Stack</h3>
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {selectedProject.technologies.map((tech: string, idx: number) => (
                        <motion.span
                          ...
                          className="px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-md bg-gradient-to-r from-cyan-100 to-sky-100 dark:from-cyan-900/40 dark:to-sky-900/40 text-cyan-700 dark:text-cyan-300 text-xs sm:text-sm font-semibold border border-cyan-200/70 dark:border-cyan-800/50"
                        >
                          {tech}
                        </motion.span>
                      ))}
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {selectedProject?.github && (
                    <motion.a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-semibold text-sm sm:text-base shadow-lg hover:shadow-xl transition-all"
                  ...
                  {selectedProject?.video && (
                    <motion.button
                      ...
                      className="inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg bg-gradient-to-r from-cyan-600 to-sky-600 text-white font-semibold text-sm sm:text-base shadow-lg hover:shadow-xl transition-all"
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24">
